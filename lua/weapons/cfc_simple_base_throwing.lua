...
function SWEP:CanThrow()
    if self:GetFinishThrow() > 0 then return false end
    if self:GetFinishReload() > 0 then return false end
...
if CLIENT then
    ...
    function SWEP:CustomAmmoDisplay()
        return { Draw = true, PrimaryClip = self:GetOwner():GetAmmoCount( self.Primary.Ammo ) }
    end
end
